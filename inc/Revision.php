...
class Revision
{
	...
	static function GetFile($filename)
	{
		...
		foreach ( $array as $key => $element )
		{
			if ( trim($element[0][1]) == trim($filename) )
			{
				...
				if ( count(explode('/', $element[0][1])) > 2 )
				{
					$x = explode('/', $element[0][1]);
				
					$b = $x[1].'/';
				}

				return $b.$element[1][1];
			}
		}
	...
	static function addToTable($file, $content)
	{
		...
		file_put_contents(self::$archivePath.'/cacheTable', htmlentities($file).':::!@#!@$!$$$%^^:::'.htmlentities($archive_name).':::!@#!@$!$$$%^^:::'.htmlentities(dirname($file)).':::!@#!@$!$$$%^^:::'."\n", FILE_APPEND);
